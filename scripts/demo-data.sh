...
# macOS/Linux ship python3; on Windows (Git Bash) a `python3` is ON PATH but is
# the Microsoft Store stub, which prints an install notice and exits non-zero.
# `command -v` finds it and is happy, so probing for the NAME is not enough —
# the busy-wait below then never matched and this script spun forever without
# writing a single file. Probe that the interpreter actually runs.
PY=""
for c in python3 python py; do
  if "$c" -c 'import sys' >/dev/null 2>&1; then PY=$c; break; fi
done
[ -n "$PY" ] || { echo "demo-data: no working python found" >&2; exit 1; }
...
until [ "$(curl -s "$API/trace" | "$PY" -c 'import sys,json;print(json.load(sys.stdin)["busy"])')" = "False" ]; do sleep 0.3; done

curl -s "$API/trace" > "$OUT/trace.json"
curl -s "$API/merges" > "$OUT/merges.json"
curl -s "$API/quant-sample" > "$OUT/quant-sample.json"

LAST=$("$PY" -c 'import json;print(len(json.load(open("'$OUT'/trace.json"))["tokens"])-1)')
PROD=$((LAST - 1))
for P in $(seq 0 $PROD); do curl -s "$API/lens?pos=$P&k=5" > "$OUT/lens-$P.json"; done

for ID in $("$PY" -c 'import json;print(" ".join(str(t["id"]) for t in json.load(open("'$OUT'/trace.json"))["tokens"]))'); do
...
# the softmax normalizer over a temperature grid, per producing position. Every
# candidate's logit is already in trace.json; this is the one thing a recording
# cannot reconstruct, and with it the demo's temperature dial computes exact
# full-vocabulary shares client-side instead of sitting dead (design-35).
for P in $(seq 0 $PROD); do
  curl -s "$API/odds-table?pos=$P" > "$OUT/odds-table-$P.json"
done
